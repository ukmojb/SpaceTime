...
public class TextDataRenderer {
     ...
     */
    public static int drawText(int x, int y, int boxWidth, int boxHeight, TextData[] data, int mouseX, int mouseY, FontRenderer fr) {
        List<String> tooltip = new ArrayList<String>();
        int action = drawText(x, y, boxWidth, boxHeight, data, mouseX, mouseY, fr, tooltip);
    ...
    public static int drawText(int x, int y, int boxWidth, int boxHeight, TextData[] data, int mouseX, int mouseY, FontRenderer fr, @Nonnull List<String> tooltip) {
...
//            }
            ...
            allHeight += (int) (split.length * item.scale);
        }
...
//    public static String[] cropStringBySize(String s, String modifiers, int guiWidth, int guiHeight, int firstWidth, FontRenderer fr, float scale) {
//        int curWidth = 0;
...
//
//            if(s.charAt(i) == '\n' || (curHeight == (int) (fr.FONT_HEIGHT * scale) && curWidth > firstWidth) || (curHeight != (int) (fr.FONT_HEIGHT * scale) && curWidth > guiHeight)) {
//                int oldI = i;
...
//    }
    ...
    public static String[] cropStringBySize(String s, String modifiers, int guiWidth, int guiHeight, int firstWidth, FontRenderer fr, float scale) {
        int curWidth = 0;
        int curHeight = (int) (fr.FONT_HEIGHT * scale);
        StringBuilder sb = new StringBuilder(s); // 使用StringBuilder提高字符串操作效率

        for (int i = 0; i < sb.length(); i++) {
            char c = sb.charAt(i);
            curWidth += (int) (fr.getStringWidth(modifiers + c) * scale);

            // 检查是否需要换行（换行符/宽度超限）
            boolean shouldBreak = (c == '\n') ||
                    (curHeight <= fr.FONT_HEIGHT * scale && curWidth > firstWidth) ||
                    (curHeight > fr.FONT_HEIGHT * scale && curWidth > guiWidth);

            if (shouldBreak) {
                int oldI = i;
                // 回溯查找最近的空格（单词边界）
                while (i >= 1 && sb.charAt(i) != ' ' && sb.charAt(i) != '\n') {
                    i--;
                }

                // 处理换行位置
                if (i > 0 && (sb.charAt(i) == ' ' || sb.charAt(i) == '\n')) {
                    sb.setCharAt(i, '\r'); // 替换空格为换行符
                    i++; // 跳过已被处理的空格
                } else {
                    // 找不到空格时强制插入换行
                    i = oldI;
                ...
                // 更新布局状态
                curWidth = 0;
                curHeight += (int) (fr.FONT_HEIGHT * scale);

                // 检查高度限制
                if (curHeight >= guiHeight) {
                    return sb.substring(0, i).split("\r");
                }
            }
        }
