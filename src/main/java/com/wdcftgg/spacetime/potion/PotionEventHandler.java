...
import com.wdcftgg.spacetime.potion.buff.BasePotion;
...
//            }
            ...
            if (!(trueSource instanceof EntityPlayer)) {//Players have their own critical judgement system. Now we add the non-player system.
                ...
                if (isCritical) {
                    ...
                    //SpaceTime.Log(String.format("%s:isCrit = %s, x%s =%s, ", trueSource.getName(), isCritical, critDmg, evt.getAmount()));
                }
                //SpaceTime.Log(String.format("%s:isCrit = %s, x1f =%s, ", trueSource.getName(), isCritical, evt.getAmount()));
            }
        }
