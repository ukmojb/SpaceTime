package com.wdcftgg.spacetime.item.weapon;

import java.util.List;

import com.wdcftgg.spacetime.item.ItemAdaptingBase;
...
public class ItemHealingGun extends ItemAdaptingBase {
    ...
    {
        //SpaceTime.Log("onCreatureStoppedUsing");
        ...
        //SpaceTime.Log("found %d targets", entities.size());
    ...
    void spawnParticles(EnumParticleTypes particleType, World worldIn, EntityLivingBase center)
    {
        ...
        for (float angle = 0f; angle < maxAngle; angle += deltaAngle)
        {
           // float radius = center.getRNG().nextFloat() * speedMagnitude;
            //SpaceTime.Log("spawn particle at %s, %s, %s", center.posX, center.posY, center.posZ);
